...
def test_cnm_spec_is_tracked_and_not_authorized() -> None:
    spec_path = ROOT / "docs" / "research" / "CGRN_HSR_CNM_RESEARCH_SPEC.md"
...
def test_level2b_closure_does_not_authorize_runtime_controller() -> None:
    payload = _load("results/level2b0/final_verdicts.json")
    assert payload["portable_external_context_controller"]["implementation_authorized"] is False
    l10 = next(row for row in payload["seam_L_decomposition"] if row["id"] == "L10")
    assert l10["verdict"] == "PROTOTYPE"
    assert l10["status"] == "DEFERRED"
    assert l10["implementation_authorized"] is False
